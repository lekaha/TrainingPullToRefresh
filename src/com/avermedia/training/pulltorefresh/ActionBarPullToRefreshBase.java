...
public abstract class ActionBarPullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	public ActionBarPullToRefreshBase(Context context) {
		super(context);
	...
	public ActionBarPullToRefreshBase(Context context, AttributeSet attrs) {
		super(context, attrs);
	...
	public boolean isWindowManagerExist() {
		return (null != mWindowManager);
	}
	...
	protected void onPullToRefresh() {
		addHeaderViewToActionBar(mHeaderLayout);
	...
	protected void onRefreshing(final boolean doScroll) {
		...
		//If you want it is invisible need to remove it from this window.
		removeHeaderViewFromActionBar(mHeaderLayout);
	...
	protected final void refreshLoadingViewsSize() {
		...
		int pLeft = mHeaderLayout.getPaddingLeft();
		int pTop = mHeaderLayout.getPaddingTop();
		int pRight = mHeaderLayout.getPaddingRight();
		int pBottom = mHeaderLayout.getPaddingBottom();
	...
	protected final void setHeaderScroll(int value) {
		...
		updateHeaderViewPosition(mHeaderLayout);
	}
	...
	/**
	 * Attach the header view to action bar if this view hasn't window token.
	 * @param headerView
	 */
    protected void addHeaderViewToActionBar(View headerView) {
    	
    	if (null != headerView && null == headerView.getWindowToken()) {
	        ...
	        WindowManager.LayoutParams wlp = getLoadingLayoutLayoutParams();
	        wlp.x = 0;
	        wlp.y = mRect.top;
	        ...
	        try {
	        	getWindowManager().addView(headerView, wlp);
	        }
	        catch(BadTokenException e) {
	        	// TODO: Here has a issue that will cause crash.
	        }
    	}
    	...
    	if(null != headerView) {
	        ...
	        if (wlp != null && wlp.y != mRect.top) {
	            ...
	            try {
	            	...
	            	addHeaderViewToActionBar(headerView);
	            }
	        }
    	}
    }
    ...
    /**
     * Remove the header view to action bar if this view has window token.
     * @param headerView
     */
    protected void removeHeaderViewFromActionBar(View headerView) {
        if (null != headerView && null != headerView.getWindowToken()) {
        	getWindowManager().removeViewImmediate(headerView);
        }
    }
	...
	private WindowManager.LayoutParams getLoadingLayoutLayoutParams() {
	...
	final class SmoothScrollRunnable implements Runnable {
		...
		public void run() {
			...
			if (mContinueRunning && mScrollToY != mCurrentY) {
				ViewCompat.postOnAnimation(ActionBarPullToRefreshBase.this, this);
			} else {
				if (null != mListener) {
